...
export const Prospects: React.FC<ProspectsProps> = ({ prospects, setProspects, prospectLists, setProspectLists, deals, setDeals, products }) => {
    const { create: canCreate, edit: canEdit, delete: canDelete } = usePermissions('Prospects');
                            ...
                            {selectedProspects.length > 0 ? (
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        {canCreate && <button onClick={() => setAddToListModalOpen(true)} className="flex items-center px-3 py-1.5 text-sm bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm hover:bg-gray-50 dark:hover:bg-slate-600"><List className="h-4 w-4 mr-2" /> Add to List</button>}
                                        {canEdit && <button onClick={() => setTaggingModalOpen(true)} className="flex items-center px-3 py-1.5 text-sm bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm hover:bg-gray-50 dark:hover:bg-slate-600"><Tag className="h-4 w-4 mr-2" /> Add Tags</button>}
                                        {canDelete && <button onClick={handleDeleteSelected} className="flex items-center px-3 py-1.5 text-sm bg-red-50 dark:bg-red-500/10 text-red-600 border border-red-200 dark:border-red-500/20 rounded-md shadow-sm hover:bg-red-100 dark:hover:bg-red-500/20"><Trash2 className="h-4 w-4 mr-2" /> Delete</button>}
                                    </div>
                                </div>
                ...
                {activeView === 'board' && (
                     <div className="flex-grow flex gap-4 overflow-x-auto pb-4">
                        {statusOptions.map(status => (
                            <div key={status} onDrop={(e) => canEdit && handleDrop(e, status)} onDragOver={canEdit ? handleDragOver : undefined} onDragLeave={canEdit ? handleDragLeave : undefined} className="w-72 flex-shrink-0 bg-slate-100 dark:bg-slate-900/50 rounded-lg flex flex-col transition-colors">
                                ...
                                <div className="p-2 space-y-2 overflow-y-auto flex-grow">
                                    {(prospectsByStatus[status] || []).map(prospect => {
                                        ...
                                        return (
                                            <Card key={prospect.id} draggable={canEdit} onDragStart={(e) => canEdit && handleDragStart(e, prospect.id)} onClick={() => handleRowClick(prospect)} className="cursor-pointer hover:ring-2 hover:ring-blue-500">
