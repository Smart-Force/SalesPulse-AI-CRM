...
export const LeadGeneration: React.FC<LeadGenerationProps> = ({ onAddProspects, prospects }) => {
    const { create: canCreate } = usePermissions('Lead Generation');
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            addToast('An error occurred during the search. Please check the console.', 'error');
        } finally {
    ...
    const handleAddSelectedToProspects = () => {
        ...
        if (newUniqueResults.length === 0) {
            addToast('No new prospects to add. All selected leads are duplicates.', 'info');
            return;
        }
        ...
        addToast(`${newProspects.length} new prospect(s) added successfully!`, 'success');
        if (duplicateCount > 0) {
            addToast(`${duplicateCount} duplicate(s) were ignored.`, 'info');
        }
        setSelectedIds([]);
    };
    ...
    const handleGenerateOutreach = async (prospectId: string) => {
        ...
        try {
            ...
            console.error(error);
            addToast('Failed to generate outreach plan.', 'error');
    ...
    const renderSkeletonTable = () => (
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead className="bg-gray-50 dark:bg-slate-800/50">
                <tr>
                    <th scope="col" className="p-4"><Skeleton className="h-4 w-4" /></th>
            ...
            <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                {Array.from({ length: searchCriteria.count }).map((_, i) => (
                    <tr key={i}>
                        <td className="p-4"><Skeleton className="h-4 w-4" /></td>
                        <td className="px-4 py-4"><Skeleton className="h-4 w-32" /><Skeleton className="h-3 w-24 mt-2" /></td>
                        <td className="px-4 py-4"><Skeleton className="h-4 w-24" /><Skeleton className="h-3 w-20 mt-2" /></td>
                        <td className="px-4 py-4"><Skeleton className="h-3 w-40" /><Skeleton className="h-3 w-24 mt-2" /></td>
                        <td className="px-4 py-4"><Skeleton className="h-3 w-20" /><Skeleton className="h-3 w-16 mt-2" /></td>
    ...
    return (
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <CardContent>
                    <form onSubmit={handleSearch}>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
                            ...
                            <button type="submit" disabled={isSearching || !canCreate} className="w-full bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            {(hasSearched || isSearching) && (
                <>
                {searchSources.length > 0 && !isSearching && (
                    <Card className="mb-4">
                        <CardHeader>
                ...
                <Card>
                    <CardHeader className="p-4 border-b border-gray-200 dark:border-slate-700 min-h-[60px] flex justify-between items-center">
                        <CardTitle className="text-lg">Master Data Table ({isSearching ? '...' : searchResults.length} Results)</CardTitle>
                        {selectedIds.length > 0 ? (
                           <button onClick={handleAddSelectedToProspects} disabled={!canCreate} className="bg-green-600 text-white font-semibold py-1.5 px-4 rounded-lg hover:bg-green-700 transition-colors flex items-center shadow-sm text-sm disabled:opacity-50">
                                <UserPlus className="h-4 w-4 mr-2" />
                                Add {selectedIds.length} to Prospects
                            </button>
                        ) : (
                             <button onClick={handleExport} disabled={searchResults.length === 0 || isSearching} className="bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-200 font-semibold py-1.5 px-4 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors flex items-center shadow-sm text-sm disabled:opacity-50">
                    ...
                    <CardContent className="p-0">
                        <div className="overflow-x-auto">
                           {isSearching ? renderSkeletonTable() : (
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                                <thead className="bg-gray-50 dark:bg-slate-800/50">
                                    <tr>
                                                        ...
                                                        {!prospect.isOutreachGenerated ? (
                                                            <div className="flex flex-col items-center justify-center p-4 text-center">
                                                                <button
                                                                    ...
                                                                    disabled={prospect.isGeneratingOutreach || !canCreate}
