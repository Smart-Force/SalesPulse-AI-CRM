...
import { X, Bot, Zap, Building, BarChart, UserCheck, MessageSquare, Lightbulb, TrendingUp, Link, Loader2, Check, ExternalLink, BookUser, Mail, Phone, Calendar, Sparkles, ChevronRight, ChevronDown, Wand2, ClipboardEdit, PhoneCall, Plus, Clock, Save, Handshake, DollarSign, Edit } from 'lucide-react';
...
const interactionIcons: { [key: string]: React.ElementType } = {
    Email: Mail,
    Call: Phone,
    Meeting: Calendar,
...
const ProspectIntelligencePanel: React.FC<PanelProps> = ({ prospect, onClose, onUpdateProspect, deals, setDeals, products }) => {
    const { edit: canEditDeals, create: canCreateDeals } = usePermissions('Prospects');
    ...
    const contactHistory = prospect.contactHistory || [];
                            ...
                            {!prospect.isEnriched ? (
                                ...
                                <>
                                    ...
                                    <Section title="Contact History" icon={BookUser}>
                                        ...
                                        <ul className="space-y-4">
                                            {contactHistory.map((item, index) => {
                                                ...
                                                const historyKey = `${prospect.id}-hist-${index}`;
                                                const isExpanded = expandedHistoryId === historyKey;
                                                const canExpand = item.summary || item.transcript;
                                                return (
                                                <li key={historyKey} className="flex items-start space-x-4">
                                                    ...
                                                    <div className="flex-grow">
                                                        <div onClick={() => canExpand && setExpandedHistoryId(isExpanded ? null : historyKey)} className={canExpand ? 'cursor-pointer' : ''}>
                                                            <p className="font-medium text-gray-800 dark:text-slate-200 flex items-center">
                                                                {item.title || item.type} on <span className="font-semibold ml-1">{item.date}</span>
                                                                ...
                                                                {canExpand && <ChevronDown className={`h-4 w-4 ml-auto text-gray-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`} />}
                                                            </p>
                                                        ...
                                                        {isExpanded && item.summary && (
                                                            <div className="mt-2 p-2 bg-slate-100 dark:bg-slate-700/50 rounded-md">
                                                                <h5 className="font-semibold text-xs uppercase text-gray-500">Summary</h5>
                                        ...
                                        ) : <p className="text-sm text-gray-500">No contact history recorded.</p>}
                                    </Section>
                                    ...
                                    <Section title="Notes" icon={ClipboardEdit}>
                                        <textarea value={notesContent} onChange={handleNotesChange} rows={5} className="w-full p-2 border rounded-md bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-200 focus:ring-2 focus:ring-blue-500" placeholder="Add private notes..."/>
                                        <div className="flex justify-end items-center h-8">{isNotesDirty && <button onClick={handleSaveNotes} disabled={isSavingNotes} className="inline-flex items-center bg-blue-600 text-white px-3 py-1 rounded-lg font-semibold text-sm disabled:opacity-50">{isSavingNotes ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : 'Save'}</button>} {saveNotesStatus === 'success' && !isNotesDirty && <span className="text-sm text-green-600 flex items-center"><Check className="h-4 w-4 mr-1"/>Saved!</span>}</div>
                                    </Section>
                                </>
                            )}
                          </>
                        ...
                        {activeTab === 'deals' && (
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Deals ({deals.length})</h3>
                                    {canCreateDeals && <button onClick={() => handleOpenDealBuilder(null)} className="inline-flex items-center bg-blue-600 text-white px-3 py-1.5 rounded-lg font-semibold text-sm"><Plus className="mr-2 h-4 w-4"/>New Deal</button>}
                                </div>
                                {deals.length > 0 ? (
                                    <div className="space-y-3">
                                        {deals.map(deal => {
                                                    ...
                                                    <div className="p-4 flex justify-between items-center cursor-pointer" onClick={() => toggleDealExpansion(deal.id)}>
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                <span className={`px-2 inline-flex text-xs font-semibold rounded-full ${dealStatusColors[deal.status]}`}>{deal.status}</span>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center gap-4">
                                                            <p className="text-xl font-bold">{formatCurrency(dealValue)}</p>
                                                            {canEditDeals && <button onClick={e => { e.stopPropagation(); handleOpenDealBuilder(deal); }} className="p-2 text-gray-500 hover:text-blue-600 rounded-full"><Edit className="h-4 w-4" /></button>}
                                                        </div>
                                                    </div>
                                                    {isExpanded && (
                                                        <div className="pb-4 px-4 border-t border-gray-200 dark:border-slate-600"><table className="min-w-full text-sm mt-3"><thead><tr className="text-xs text-gray-500 uppercase"><th className="pb-2 text-left w-2/5">Product</th><th className="pb-2 text-right">Final Price</th><th className="pb-2 text-right">Agent Comm.</th></tr></thead><tbody className="divide-y dark:divide-slate-600">{deal.lineItems.map(item => <tr key={item.id}><td className="py-2">{item.name}</td><td className="py-2 text-right">{formatCurrency(calculateDealValue({ lineItems: [item] } as Deal))}</td><td className={`py-2 text-right ${calculateDealValue({ lineItems: [item] } as Deal) - item.basePrice < 0 ? 'text-red-500' : ''}`}>{formatCurrency(calculateDealValue({ lineItems: [item] } as Deal) - item.basePrice)}</td></tr>)}</tbody></table></div>
                                                    )}
                                                </div>
                                            );
                                        })}
                                    ...
                                    <div className="text-center py-10 border-2 border-dashed rounded-lg"><DollarSign className="mx-auto h-10 w-10 text-gray-400"/><p className="mt-2 font-semibold">No deals yet</p><p className="text-sm text-gray-500">Create a new deal to start tracking.</p></div>
                                )}
                            </div>
                        )}
                    </div>
