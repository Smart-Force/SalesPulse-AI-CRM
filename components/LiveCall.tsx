...
import type { Prospect, TranscriptEntry } from '../types';
import { summarizeCallTranscript } from '../services/aiService';
import { Card, CardContent, CardHeader, CardTitle } from './ui/Card';
import { Phone, Mic, MicOff, Bot, User, Loader2, Lightbulb, TrendingUp, CheckCircle, FileText, AlertCircle, CircleDot } from 'lucide-react';
...
export const LiveCall: React.FC<LiveCallProps> = ({ prospects }) => {
    ...
    const handleStartCall = async () => {
        if (!selectedProspectId) {
            alert('Please select a prospect for the call.');
            return;
        }
        ...
        try {
            ...
            sessionPromiseRef.current = ai.live.connect({
                ...
                config: {
                    ...
                    systemInstruction: 'You are a friendly and helpful AI sales assistant. Your role is to act as the prospect in a practice sales call. Respond naturally to the user.',
    ...
    const handleEndCall = async (isClosedByServer = false) => {
        ...
        const fullTranscript = transcript.map(t => `${t.speaker === 'user' ? 'Sales Rep' : 'Prospect'}: ${t.text}`).join('\n');
                            ...
                            {callState === 'processing_summary' || callState === 'ended' ? (
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <div className="p-4 bg-slate-100 dark:bg-slate-700/50 rounded-lg text-sm text-gray-700 dark:text-slate-300 prose prose-sm dark:prose-invert max-w-none">{summary || '...'}</div>
                                    </div>
                                    <div>
